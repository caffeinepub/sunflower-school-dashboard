...
function ReceiptModal({
  ...
  const unpaidTuitionMonths = activeMonths.filter((m) => !student.fees[m]);
  const unpaidGenMonths = activeMonths.filter((m) => !student.generatorFees[m]);
  const unpaidTransMonths = activeMonths.filter(
    (m) => !student.transportFees[m],
  );
  const unpaidExamMonths = activeMonths.filter((m) => !student.examFees?.[m]);
  ...
  // Charge visibility & paid status
  const showGen = student.generatorCharge > 0;
  const showTrans = student.transportCharge > 0;
  const showExam = (student.examCharge ?? 0) > 0;

  const tuitionPaid = paidTuitionMonths.length > 0;
  const genPaid = paidGenMonths.length > 0;
  const transPaid = paidTransMonths.length > 0;
  const examPaid = paidExamMonths.length > 0;

  const unpaidCharges: string[] = [];
  if (unpaidTuitionMonths.length > 0) {
    if (!tuitionPaid) {
      unpaidCharges.push(
        `Tuition Fee — ${INR}${fmt(student.classFee)}/mo (no months paid)`,
      );
    } else {
      unpaidCharges.push(
        `Tuition Fee — Not paid for: ${unpaidTuitionMonths.join(", ")} (${INR}${fmt(student.classFee)}/mo each)`,
      );
    }
  }
  if (showGen && unpaidGenMonths.length > 0) {
    if (!genPaid) {
      unpaidCharges.push(
        `Generator Charge — ${INR}${fmt(student.generatorCharge)}/mo (no months paid)`,
      );
    } else {
      unpaidCharges.push(
        `Generator Charge — Not paid for: ${unpaidGenMonths.join(", ")} (${INR}${fmt(student.generatorCharge)}/mo each)`,
      );
    }
  }
  if (showTrans && unpaidTransMonths.length > 0) {
    if (!transPaid) {
      unpaidCharges.push(
        `Transport Charge — ${INR}${fmt(student.transportCharge)}/mo (no months paid)`,
      );
    } else {
      unpaidCharges.push(
        `Transport Charge — Not paid for: ${unpaidTransMonths.join(", ")} (${INR}${fmt(student.transportCharge)}/mo each)`,
      );
    }
  }
  if (showExam && unpaidExamMonths.length > 0) {
    if (!examPaid) {
      unpaidCharges.push(
        `Examination Fee — ${INR}${fmt(student.examCharge ?? 0)}/sitting (no months paid)`,
      );
    } else {
      unpaidCharges.push(
        `Examination Fee — Not paid for: ${unpaidExamMonths.join(", ")} (${INR}${fmt(student.examCharge ?? 0)}/sitting each)`,
      );
    }
  }

  const allPaid = unpaidCharges.length === 0;
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .receipt { max-width: 600px; margin: 0 auto; border: 2px solid #000; padding: 24px; }
          ...
          .fees-table { width: 100%; border-collapse: collapse; margin-bottom: 4px; font-size: 13px; }
          ...
          .fees-table td:first-child { background: #000 !important; color: #fff !important; font-weight: bold; }
          ...
          .paid-badge { background: #e6f9f0; color: #006b3c; border: 1px solid #00c060; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 11px; }
          .unpaid-badge { background: #fff0f0; color: #cc0000; border: 1px solid #ff4444; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 11px; }
          .unpaid-row { background: #fff8f8; }
          .unpaid-box { background: #fff5f5; border: 1px solid #ffcccc; padding: 10px; margin-top: 12px; border-radius: 4px; }
          .unpaid-box-title { color: #cc0000; font-size: 11px; letter-spacing: 1px; text-transform: uppercase; font-weight: bold; margin-bottom: 6px; }
          .unpaid-box-item { font-size: 12px; color: #cc0000; margin-bottom: 2px; }
          .all-paid-box { background: #f0fff8; border: 1px solid #00c060; color: #006b3c; text-align: center; font-weight: bold; font-size: 13px; padding: 8px; margin-top: 12px; border-radius: 4px; }
        ...
        <div className="p-5">
          <div id="fee-receipt-print">
            ...
            >
              ...
              {/* Fees Table — shows ALL applicable charges with PAID/UNPAID status */}
              <table
                style={{
                  width: "100%",
              ...
              >
                ...
                <tbody>
                  {/* Tuition — always shown */}
                  <tr
                    style={{
                      borderBottom: "1px solid #eee",
                      background: tuitionPaid ? "#fff" : "#fff8f8",
                    }}
                  >
                    <td
                      style={{
                    ...
                    >
                      {paidTuitionMonths.length > 0 && (
                        <div style={{ color: "#006b3c" }}>
                          ✓ Paid: {paidTuitionMonths.join(", ")}
                        </div>
                      )}
                      {unpaidTuitionMonths.length > 0 ? (
                        <div style={{ color: "#cc0000" }}>
                          ✗ Not Paid: {unpaidTuitionMonths.join(", ")}
                        </div>
                      ) : paidTuitionMonths.length > 0 ? (
                        <div style={{ color: "#006b3c" }}>None unpaid</div>
                      ) : (
                        <div style={{ color: "#cc0000" }}>No months paid</div>
                      )}
                    </td>
                    <td
                      style={{
                    ...
                    >
                      {tuitionPaid ? `${INR} ${fmt(tuitionTotal)}` : "Nil"}
                    </td>
                    <td style={{ padding: "8px 10px", textAlign: "center" }}>
                      {tuitionPaid ? paidBadge : unpaidBadge}
                    </td>
                  </tr>

                  {/* Generator — only if charge > 0 */}
                  {showGen && (
                    <tr
                      style={{
                        borderBottom: "1px solid #eee",
                        background: genPaid ? "#fff" : "#fff8f8",
                      }}
                    >
                      <td
                        style={{
                      ...
                      >
                        {paidGenMonths.length > 0 && (
                          <div style={{ color: "#006b3c" }}>
                            ✓ Paid: {paidGenMonths.join(", ")}
                          </div>
                        )}
                        {unpaidGenMonths.length > 0 ? (
                          <div style={{ color: "#cc0000" }}>
                            ✗ Not Paid: {unpaidGenMonths.join(", ")}
                          </div>
                        ) : paidGenMonths.length > 0 ? (
                          <div style={{ color: "#006b3c" }}>None unpaid</div>
                        ) : (
                      ...
                      >
                        {genPaid ? `${INR} ${fmt(genTotal)}` : "Nil"}
                      </td>
                      <td style={{ padding: "8px 10px", textAlign: "center" }}>
                        {genPaid ? paidBadge : unpaidBadge}
                      </td>
                    </tr>
                  )}

                  {/* Transport — only if charge > 0 */}
                  {showTrans && (
                    <tr
                      style={{
                        borderBottom: "1px solid #eee",
                        background: transPaid ? "#fff" : "#fff8f8",
                      }}
                    >
                      <td
                        style={{
                      ...
                      >
                        {paidTransMonths.length > 0 && (
                          <div style={{ color: "#006b3c" }}>
                            ✓ Paid: {paidTransMonths.join(", ")}
                          </div>
                        )}
                        {unpaidTransMonths.length > 0 ? (
                          <div style={{ color: "#cc0000" }}>
                            ✗ Not Paid: {unpaidTransMonths.join(", ")}
                          </div>
                        ) : paidTransMonths.length > 0 ? (
                          <div style={{ color: "#006b3c" }}>None unpaid</div>
                        ) : (
                      ...
                      >
                        {transPaid ? `${INR} ${fmt(transTotal)}` : "Nil"}
                      </td>
                      <td style={{ padding: "8px 10px", textAlign: "center" }}>
                        {transPaid ? paidBadge : unpaidBadge}
                      </td>
                    </tr>
                  )}

                  {/* Exam — only if charge > 0 */}
                  {showExam && (
                    <tr
                      style={{
                        borderBottom: "1px solid #eee",
                        background: examPaid ? "#fff" : "#fff8f8",
                      }}
                    >
                      <td
                        style={{
                      ...
                      >
                        {paidExamMonths.length > 0 && (
                          <div style={{ color: "#006b3c" }}>
                            ✓ Paid: {paidExamMonths.join(", ")}
                          </div>
                        )}
                        {unpaidExamMonths.length > 0 ? (
                          <div style={{ color: "#cc0000" }}>
                            ✗ Not Paid: {unpaidExamMonths.join(", ")}
                          </div>
                        ) : paidExamMonths.length > 0 ? (
                          <div style={{ color: "#006b3c" }}>None unpaid</div>
                        ) : (
                      ...
                      >
                        {examPaid ? `${INR} ${fmt(examTotal)}` : "Nil"}
                      </td>
                      <td style={{ padding: "8px 10px", textAlign: "center" }}>
                        {examPaid ? paidBadge : unpaidBadge}
                      </td>
                    </tr>
                  )}
